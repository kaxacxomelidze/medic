<?php
// --- Secure session cookie before starting the session ---
...
/* --- UPDATE PAYMENT (method/amount/order/date for non-donor payments) --- */
if ($_SERVER['REQUEST_METHOD']==='POST' && (($_POST['action']??'')==='update_payment')) {
  ...
  if (!in_array($method, ['cash','bog','transfer'], true)) {
    echo json_encode(['status'=>'error','message'=>'ტიპი არასწორია (დონორი რედაქტირდება საგარანტიო ბლოკიდან)']); exit;
  }
  ...
  try {
    ...
    $st = $pdo->prepare("SELECT patient_id, method FROM payments WHERE id=? FOR UPDATE");
    $st->execute([$pay_id]);
    $row = $st->fetch(PDO::FETCH_ASSOC);
    ...
    if (($row['method'] ?? '') === 'donor') {
      throw new RuntimeException('დონორის გადახდა რედაქტირდება საგარანტიო ბლოკიდან');
    }
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  json_guard_auth();
  csrf_guard(); // ✅ enforce CSRF for ALL POSTs
...
<table>
  ...
  /* --- CREATE INVOICE DRAFT (header + first line L1) --- */
  if ($action === 'create_invoice_draft') {
    ...
    try{
      ...
      // Insert first line (L1). Try WITH description then fallback WITHOUT.
      try {
        $insIt = $pdo->prepare("
          INSERT INTO invoice_items (invoice_id, patient_service_id, description, quantity, unit_price, line_total, comment)
          VALUES (?,?,?,?,?,?,?)
        ");
        $insIt->execute([$invoice_id, null, $init_title, $init_qty, $init_price, $init_sum, $init_comment]);
      } catch (PDOException $e) {
        $msg = $e->getMessage();
        if ($e->getCode() === '42S22' || stripos($msg, 'Unknown column') !== false) {
          $fallbackComment = ($init_title ? ($init_title . "\n") : '') . $init_comment;
          $insIt2 = $pdo->prepare("
            INSERT INTO invoice_items (invoice_id, patient_service_id, quantity, unit_price, line_total, comment)
            VALUES (?,?,?,?,?,?)
          ");
          $insIt2->execute([$invoice_id, null, $init_qty, $init_price, $init_sum, $fallbackComment]);
        } else {
          throw $e;
        }
      }

      // total is 0 for now (only L1 with 0.00)
      $pdo->commit();
              ...
              <tr id="O<?= (int)$pr['id'] ?>">
                ...
                <td>
                  <button type="button"
                          ...
                          <?= (strtolower($pr['method'])==='donor' ? 'data-donor="1" title="დონორის გადახდა რედაქტირდება საგარანტიო ბლოკიდან"' : '') ?>>
                    რედაქტირება
                  </button>
                </td>
              </tr>
...
<div class="container">
  <div class="mainContent">
    <ul class="tabs">
        ...
        <li><a href="patient_hstory.php" class="active">პაციენტის ისტორია</a></li>
...
<div
  ...
  aria-live="polite"
  aria-atomic="true"
  aria-label="Patient details and actions"
>
...
<!-- OVERLAY FOR MODALS (pay, edit, etc.) -->
<div id="payOverlay" role="dialog" aria-modal="true" aria-label="Pay dialog" style="display:none;"></div>

<!-- jQuery (already included in your page, keep once) -->
...
function getCSRF(){
  ...
  const t1 = $rs.data('csrf'); if (t1) return String(t1);
  const meta = document.querySelector('meta[name="csrf-token"]');
  if (meta && meta.content) return meta.content;
  if (window.CSRF_TOKEN) return String(window.CSRF_TOKEN);
  return '';
}
function toggleAction($a, enable){
  if (enable) {
    $a.removeClass('disabled').attr({'aria-disabled':'false','tabindex':'0'});
  } else {
    $a.addClass('disabled').attr({'aria-disabled':'true','tabindex':'-1'});
  }
}
function showSpinner(on){
  const $sp = $('#loadingIndicator');
  $sp.css('display', on ? 'inline-block' : 'none');
}

/* small text escaper (avoid innerHTML when possible) */
function setText($el, text){
  if (!$el || !$el[0]) return;
  $el[0].textContent = text != null ? String(text) : '';
}

/* ===================== Active Patient Wiring ===================== */
function setActivePatientUI(info){
  // info: { id, first, last, personal }
  const pid = Number(info.id||0);
  $('#hdPtRgID').val(pid > 0 ? pid : '');
  if (pid > 0) {
    setText($('#fullname'), `${info.first||''} ${info.last||''} — ${info.personal||''}`);
    toggleAction($('#mr_pay'), true);
    toggleAction($('#mr_rpinvo'), true);
    // placeholders; enable only if you actually implement click handlers below
    toggleAction($('#mr_chkout'), true);
    toggleAction($('#mr_rwrt'), true);
    toggleAction($('#mr_rpxcalc'), true);
    toggleAction($('#mr_rpxcfqt'), true);
    toggleAction($('#mr_prcwages'), true);

    // Load history for this patient
    loadPatientHistory(pid);
  } else {
    setText($('#fullname'), 'აირჩიეთ პაციენტი სიისგან, რომ ნახოთ დეტალები.');
    ['#mr_pay','#mr_rpinvo','#mr_chkout','#mr_rwrt','#mr_rpxcalc','#mr_rpxcfqt','#mr_prcwages'].forEach(sel=>{
      toggleAction($(sel), false);
    });
    $('#patientHistory').hide();
  }
}

/* Pulls patient services via GET action=get_services */
function loadPatientHistory(pid){
  $('#patientHistory').show();
  setText($('#historyContent'), 'იტვირთება...');
  showSpinner(true);
  $.ajax({
    url: `${location.pathname}?action=get_services&id=${encodeURIComponent(pid)}`,
    method: 'GET',
    dataType: 'json'
  }).done(function(rows){
    if (!Array.isArray(rows) || rows.length===0){
      setText($('#historyContent'), 'ჩანაწერი არ არის.');
      return;
    }
    // Build a simple table
    const $tbl = $('<table/>').addClass('simple-table').css({width:'100%',borderCollapse:'collapse',fontSize:'14px'});
    ...
    rows.forEach((r, i) => {
      $tbody.append(
        $('<tr/>').append(
          $('<td/>').text(i+1),
          $('<td/>').text(r.service_name||'—'),
          $('<td/>').text(nf2(r.quantity||1)),
          $('<td/>').text(nf2(r.unit_price||0)),
          $('<td/>').text(nf2(r.sum||0)),
          $('<td/>').text(r.doctor_name||'—'),
          $('<td/>').text(r.created_at || '—')
        )
      );
    });
    $tbl.append($thead, $tbody);
    $('#historyContent').empty().append($tbl);
  }).fail(function(){
    setText($('#historyContent'), 'შეცდომა ჩატვირთვისას.');
  }).always(function(){
    showSpinner(false);
  });
}
...
function openPayModal(pid){
  const $ov = $('#payOverlay');
  $ov.show().attr('aria-hidden','false').html(
    '<div class="modal" style="position:relative;background:#fff;border-radius:10px;box-shadow:0 10px 26px rgba(0,0,0,.25);width:960px;max-width:calc(100% - 32px);max-height:calc(100vh - 64px);overflow:auto;padding:16px;border:1px solid #e8e8e8;margin:32px auto;">'+
      '<button class="close" data-close style="position:absolute;top:10px;right:10px;width:34px;height:34px;border:1px solid #ddd;border-radius:50%;background:#f7f7f7;cursor:pointer">&times;</button>'+
      '<div style="padding:12px">იტვირთება...</div>'+
    '</div>'
  );

  // close on backdrop click
  $ov.off('click.rs').on('click.rs', function(e){
    if (e.target === $ov[0]) closeOverlay();
  });

  // ESC to close
  $(document).off('keydown.rs').on('keydown.rs', function(e){
    if (e.key === 'Escape') closeOverlay();
  });

  // load content
  $.ajax({
    url: `${location.pathname}?action=pay_view&id=${encodeURIComponent(pid)}`,
    method: 'GET',
    dataType: 'html'
  }).done(function(html){
    $ov.html(html);
    // make sure close button closes
    $ov.find('.close').on('click', closeOverlay);
  }).fail(function(){
    $ov.html(
      '<div class="modal" style="position:relative;background:#fff;border-radius:10px;box-shadow:0 10px 26px rgba(0,0,0,.25);width:520px;max-width:calc(100% - 32px);padding:16px;border:1px solid #e8e8e8;margin:32px auto;">'+
      '<button class="close" data-close style="position:absolute;top:10px;right:10px;width:34px;height:34px;border:1px solid #ddd;border-radius:50%;background:#f7f7f7;cursor:pointer">&times;</button>'+
      '<div style="padding:12px;color:#a00">შეცდომა ჩატვირთვისას</div>'+
      '</div>'
    );
    $ov.find('.close').on('click', closeOverlay);
  });
...
function closeOverlay(){
  const $ov = $('#payOverlay');
  $ov.hide().attr('aria-hidden','true').empty();
  $(document).off('keydown.rs');
}

/* expose to global in case other code calls it */
window.openPayModal = openPayModal;
window.closePayModal = closeOverlay;

/* ===================== Payments: summary recalculation ===================== */
function recalcPaymentsSummary(){
  let paidReal = 0;
  $('#pa_pa tbody tr').each(function(){
    const mtxt = $(this).find('td').eq(2).text().trim().toLowerCase();
    const amt = parseFloat(($(this).find('td').eq(4).text()||'').replace(/[^0-9.]/g,''))||0;
    // count only non-donor real payments (cash/bog/transfer); donor is excluded
    if (!/^donor\b/.test(mtxt)) paidReal += amt;
  });
  $('#pa_gadax').val(nf2(paidReal));
}
window.recalcPaymentsSummary = recalcPaymentsSummary;

/* ===================== Edit Payment Dialog (delegated) ===================== */
...
$(document).on('click', '#ep_save', function () {
  ...
  .then(j => {
    ...
    // Update the payments row in the pay modal
...
// Open "edit payment" dialog from the Pay overlay table
$(document).on('click', '#payOverlay .edit-payment', function () {
  const $btn    = $(this);
  if ($btn.data('donor')) { alert('დონორის გადახდა რედაქტირდება საგარანტიო ბლოკიდან.'); return; }
  ...
  const html = `
    <div id="dlgEditPay" data-pay-id="${payId}" data-pid="${pid}"
      ...
      <div style="background:#fff;border:1px solid #ddd;border-radius:10px;box-shadow:0 12px 30px rgba(0,0,0,.25);
                  width:480px;max-width:calc(100vw - 40px);padding:16px;position:relative;">
        <button class="close" id="ep_cancel"
                style="position:absolute;top:10px;right:10px;width:34px;height:34px;border:1px solid #ddd;border-radius:50%;
                       background:#f7f7f7;cursor:pointer" aria-label="დახურვა">&times;</button>
        ...
        <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:14px">
          ...
          <button id="ep_save" class="pay-btn" style="padding:8px 12px;border:none;border-radius:6px;background:#21c1a6;color:#fff;font-weight:700;cursor:pointer">შენახვა</button>
        </div>
      </div>
    </div>`;
...
$('#mr_pay').on('click', function(e){
  if ($(this).hasClass('disabled')) return e.preventDefault();
  const pid = Number($('#hdPtRgID').val()||0);
  if (pid>0) openPayModal(pid);
  e.preventDefault();
});

$('#mr_rpinvo').on('click', function(e){
  if ($(this).hasClass('disabled')) return e.preventDefault();
  const pid = Number($('#hdPtRgID').val()||0);
  if (pid>0) {
    // Navigate to your invoices screen filtered by patient
    location.href = `angarishebi.php?patient_id=${encodeURIComponent(pid)}`;
  }
  e.preventDefault();
});

// Placeholders: wire to your real flows when ready
$('#mr_chkout, #mr_rwrt, #mr_rpxcalc, #mr_rpxcfqt, #mr_prcwages').on('click', function(e){
  if ($(this).hasClass('disabled')) return e.preventDefault();
  alert('მალე დაემატება.');
  e.preventDefault();
});

/* ===================== Hook up table selection ===================== */
$(document).on('click', '#patientsTable tbody tr[data-id]', function(){
  $('#patientsTable tbody tr').removeClass('selected');
  ...
  const info = {
    ...
    personal: $(this).data('personalid')|| ''
  };
  setActivePatientUI(info);
});

/* Support programmatic selection via custom event */
document.addEventListener('patient:selected', function(ev){
  if (!ev.detail) return;
  setActivePatientUI(ev.detail);
});
    ...
    <div class="content">
      ...
      <table class="items">
        ...
        <tbody>
          ${rows || `<tr>
            <td class="c">1</td>
            <td>PR06 მეტყველების თერაპია [RL]
...
function wirePaymentModalRoot(){
  ...
  $root.on('click', '#btn_print_selected', ()=>{
    ...
    picked.each((_,tr)=>{
      ...
      lines.push({ title: titleCell, qty, price, sum, comment: baseComment });
    });
...
// ========= /Form 100/a renderer =========
  ...
  tableBody.addEventListener('click', async (e)=>{
    ...
    const items = (j.items||[]).map(x=>({
      title: (x.description && String(x.description).trim())
        ? x.description
  ...
  function mountBuilder(invId, initial){
    ...
    const initComment = initial?.comment ?? `სრული ღირებულება-----
სახელმწიფოს ლიმიტი------
სახელმწიფოს მიერ ანაზღაურებული თანხა---
პაციენტის მიერ ასანაზღაურებელი თანხა----`;
          ...
          <tbody>
            <tr>
              ...
              <td width="5%" rowspan="3" style="vertical-align:bottom">
                <input id="${ids.add}" type="button" style="width:40px;height:40px" class="smadbut" title="შენახვა (ხაზი)">
              </td>
            </tr>
