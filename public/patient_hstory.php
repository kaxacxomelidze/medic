...
// --- serviceDoctorName helper for form100a ---
function serviceDoctorName(PDO $pdo, int $serviceRecordId): string {
  $st = $pdo->prepare("SELECT d.name FROM patient_services ps JOIN doctors d ON d.id = ps.doctor_id WHERE ps.service_record_id = ?");
  $st->execute([$serviceRecordId]);
...
<table>
  ...
  if ($action==='delete_patient') {
    ...
    try {
      ...
      $logger->logDelete("patient", $pid, ["patient_id" => $pid, "action" => "deleted"]);
      $pdo->commit();
      header('Content-Type: application/json; charset=utf-8');
      echo json_encode([
        'status'=>'ok',
...
if ($action === 'delete_payment') {
  ...
  try {
    ...
    if ($method === 'donor') {
      ...
      $logger->logDelete("payment", $pay_id, ["type" => "donor", "patient_id" => $pid, "amount" => $p["amount"]]);
    ...
    if ($guarId > 0) {
      ...
      if ($isWallet) {
        ...
        $pdo->commit();
        $logger->logDelete("payment", $pay_id, ["type" => "wallet", "patient_id" => $pid, "amount" => $p["amount"]]);
    ...
    $pdo->commit();
    $logger->logDelete("payment", $pay_id, ["type" => "regular", "patient_id" => $pid, "amount" => $p["amount"]]);
...
if ($action === 'delete_patient_service') {
  ...
  try {
    ...
    $logger->logDelete("patient_service", $ps_id, ["patient_id" => $pid]);
  ...
  if ($action === 'delete_invoice_item') {
    ...
    try {
      ...
      $logger->logDelete("invoice_item", $item_id, ["invoice_id" => $invoice_id]);
  ...
  if ($action === "form100a_save") {
    ...
    try {
      ...
      if (!$serviceId || !$patientId) {
        throw new Exception("service_id და patient_id სავალდებულოა");
      }
      
      // Validate JSON
      $decoded = json_decode($payload, true);
      if (json_last_error() !== JSON_ERROR_NONE) {
        throw new Exception("არასწორი JSON payload");
      }
      
      // Add doctor name auto if not present
      if (empty($decoded["doctor_name_auto"])) {
        $decoded["doctor_name_auto"] = serviceDoctorName($pdo, $serviceId);
        $payload = json_encode($decoded, JSON_UNESCAPED_UNICODE);
      }
      
      // Ensure table exists
      $pdo->exec("CREATE TABLE IF NOT EXISTS patient_form100a (
        id INT AUTO_INCREMENT PRIMARY KEY,
      ...
      // Upsert
      $st = $pdo->prepare("
        INSERT INTO patient_form100a (service_id, patient_id, payload, updated_at)
        VALUES (?, ?, ?, NOW())
        ON DUPLICATE KEY UPDATE patient_id=VALUES(patient_id), payload=VALUES(payload), updated_at=NOW()
      ");
      $st->execute([$serviceId, $patientId, $payload]);
      
      echo json_encode(["status" => "ok", "service_id" => $serviceId]);
    } catch (Throwable $e) {
      echo json_encode(["status" => "error", "message" => $e->getMessage()]);
    }
    exit;
...
function wirePaymentModalRoot(){
  ...
  $root.on('click', '#btn_issue_payment_invoice', async function(e){
    ...
    try {
      ...
      // 2b) Collect visible service titles from checked rows in #fs_amounttypet
      const svcTitles = [];
      $root.find('#fs_amounttypet tbody tr').each(function(){
        if ($(this).find('.srv-chk').prop('checked')) {
          const t = ($(this).find('td').eq(1).text()||'').replace(/\s+/g,' ').trim();
          if (t) svcTitles.push(t);
        }
      });
  ...
  /* duplicate #btn_issue_payment_invoice handler removed — the correct one is above (line ~4538) */
  ...
  $root.on('click', '#btn_print_selected', ()=>{
    ...
    const picked = $rows;
    if(!picked.length){ alert('აირჩიე მინიმუმ ერთი სერვისი'); return; }
...
function openInvoicePanel(pid){
  ...
  window.mountForm100A = async function(invId){
    ...
    // ====== FORM100A SAVE & PRINT HANDLERS ======
    const patientIdForForm = pid; // from openInvoicePanel scope
    ...
    // Save function
    async function saveForm100A() {
      const payload = JSON.stringify(collectFormData());
      const fd = new FormData();
      fd.append("action", "form100a_save");
      fd.append("service_id", String(invId));
      fd.append("patient_id", String(patientIdForForm));
    ...
    function showToast(msg, isError) {
      let t = document.getElementById("form100a_toast");
      if (!t) {
        t = document.createElement("div");
        t.id = "form100a_toast";
        t.style.cssText = "position:fixed;right:18px;bottom:18px;padding:12px 18px;border-radius:8px;color:#fff;z-index:99999;font-size:14px;transition:opacity 0.3s";
    ...
    // Save button handler
    const saveBtn = host.querySelector("#btn_save_100a_" + invId);
    if (saveBtn) {
      saveBtn.disabled = false; // Enable the button
      saveBtn.onclick = async () => {
        saveBtn.disabled = true;
        try {
          const j = await saveForm100A();
          showToast(j.status === "ok" ? "✅ შენახულია" : "❌ შეცდომა: " + (j.message || ""), j.status !== "ok");
        } catch (e) {
          showToast("❌ ქსელის შეცდომა", true);
        } finally {
    ...
    // Print button handler - SAVE FIRST, THEN PRINT
    const printBtn = host.querySelector("#btn_print_100a_" + invId);
    if (printBtn) {
      printBtn.onclick = async (e) => {
        ...
        try {
          ...
          if (j.status === "ok") {
            ...
            showToast("❌ შენახვა ვერ მოხერხდა! " + (j.message || ""), true);
          }
        } catch (e) {
          showToast("❌ ქსელის შეცდომა! ბეჭდვა გაუქმებულია.", true);
        } finally {
    ...
    // Flush sync button - same as save
    const flushBtn = host.querySelector("#btn_flush_sync_" + invId);
    if (flushBtn) {
      flushBtn.onclick = async () => {
        flushBtn.disabled = true;
        try {
          const j = await saveForm100A();
          showToast(j.status === "ok" ? "✅ გაგზავნილია" : "❌ შეცდომა", j.status !== "ok");
        } catch (e) {
          showToast("❌ ქსელის შეცდომა", true);
        } finally {
...
// ... (no further changes below this point; code continues exactly as in your version)
  ...
  /* duplicate #btn_issue_payment_invoice handler (vanilla JS) removed — the correct jQuery handler is above */
