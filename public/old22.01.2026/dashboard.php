...
defined('RBAC_ALLOW_ADMIN_BYPASS') || define('RBAC_ALLOW_ADMIN_BYPASS', true); // set false to test denials
defined('RBAC_DEBUG')              || define('RBAC_DEBUG', false);
defined('RBAC_AUTO_REGISTER')      || define('RBAC_AUTO_REGISTER', false);     // set true temporarily
...
if (!($__loaded_cfg && isset($pdo) && $pdo instanceof PDO)) {
  ...
  echo 'DB config not found or $pdo not created.';
  exit;
}

// 4) RBAC gate + enforce
require_once $__root.'/inc/rbac_gate.php'; // uses the version from step 1 (no DB include inside)
requirePage($pdo); // blocks if user has no access
...
<fieldset class="panel guarantee-panel">
  ...
  vf.addEventListener('submit', submitAdmission, { once: true });
